...
try {
    ...
    $preSqls = $conn->prepare("UPDATE roles set firstLevel="."'".implode(",",$firstLevel)."'".",secondLevel="."'".implode(",",$secondLevel)."'"
                                .",thirdLevel="."'".implode(",",$thirdLevel)."'"." WHERE id=".$roleId);
    $isSucces = $preSqls->execute();

    if(!$isSucces){
        $response->meta->status = 500;
        $response->meta->message = "更改权限失败";
    }
