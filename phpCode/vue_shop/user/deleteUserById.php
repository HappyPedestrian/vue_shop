...
try {
    ...
    // 删除用户
    $delUserPreSqls = $conn->prepare("DELETE FROM `users` WHERE user_id=".$quote.$u_id.$quote);
    $isSuccuss1 = $delUserPreSqls->execute();
    //删除账号与密码
    $delAccPreSqls = $conn->prepare("DELETE FROM `account` WHERE id=".$quote.$u_id.$quote);
    $isSuccuss2 = $delAccPreSqls->execute();
    ...
    if(!$isSuccuss1 && !$isSuccuss2){
        $response->meta->status = 500;
        $response->meta->message = '删除用户失败！';
    }
    echo json_encode($response);
}
