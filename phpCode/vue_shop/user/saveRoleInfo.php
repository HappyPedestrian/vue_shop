...
// 更新用户信息
    $updateUserInfoSqls = $conn->prepare("UPDATE users set role_id=".$rId." WHERE user_id="."'".$id."'");
    $isUpdateSuccess = $updateUserInfoSqls->execute();

    
    if (!$isUpdateSuccess) {
        $response->meta->message = '保存用户信息失败';
