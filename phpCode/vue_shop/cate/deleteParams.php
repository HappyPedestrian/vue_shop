...
try {
    ...
    $preSqls = $conn->prepare("DELETE FROM `attrs` WHERE attr_id=".$attr_id);
    $success1 = $preSqls->execute();

    $delValpreSqls = $conn->prepare("DELETE FROM `goods_attr` WHERE attr_id=".$attr_id);
    $success2 = $delValpreSqls->execute();
    $response = (object) array(
        "data" => null,
        "meta" => (object) array(
            "message" => "删除参数成功",
