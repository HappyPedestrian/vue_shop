...
try {
    ...
    $selectAttrPreSqls = $conn->prepare("SELECT attr_id,attr_name FROM `attrs` where cate_id=".$cateId." And attr_sel='".$sel."'");
    $selectAttrPreSqls->execute();
    $attrResult = $selectAttrPreSqls->setFetchMode(PDO::FETCH_ASSOC);
    $attrRes = $selectAttrPreSqls->fetchAll();

    foreach($attrRes as $attrKey => $attr){
        $attr = (object) $attr;
        $selectAttrValuePreSqls = $conn->prepare("SELECT attr_values FROM `goods_attr` where goods_id=".$goodId." And attr_id=".$attr->attr_id);
        $selectAttrValuePreSqls->execute();
        $attrInfoResult = $selectAttrValuePreSqls->setFetchMode(PDO::FETCH_ASSOC);
        $attrInfoRes = $selectAttrValuePreSqls->fetchAll();
        $attr->goods_id = $goodId;
        $attr->attr_values = null;
        if(count($attrInfoRes) !== 0){
            $attrInfoRes = (object) $attrInfoRes[0];
            // // 给$attr添加value属性
            $attr->attr_values = $attrInfoRes->attr_values;
        }
         
        array_push($response->data,$attr);
    }
